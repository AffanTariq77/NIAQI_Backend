...
export class AuthController {
  ...
  async googleAuthCallback(@Req() req, @Res() res: Response) {
    try {
      ...
      // Check if this is a mobile OAuth request (has niaqi:// scheme)
      const isMobileRequest = baseRedirectUrl.startsWith("niaqi://");

      if (isMobileRequest) {
        // For mobile: Return HTML bridge to avoid OAuth redirect restrictions
        const html = `
          <!DOCTYPE html>
          <html>
          <head>
        ...
        // For web: Use standard HTTP redirect
        // ALSO set cookie for web-based auth
      ...
      if (isMobileRequest) {
        // For mobile: Return HTML bridge with error
        const html = `
          <!DOCTYPE html>
          <html>
          <head>
